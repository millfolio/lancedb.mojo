...
struct Store(Movable):
    ...
    def __init__(out self, uri: String, table: String, dim: Int) raises:
        ...
        var uri_c = _cstr(uri)
        var open_fn = self.lib.get_function[Int]("ldb_open")
        self.conn = open_fn(Int(uri_c.unsafe_ptr()))
        _ = (
            uri_c^
        )  # keep the buffer mapped across the C call (ASAP-destruction)
        if self.conn == 0:
            self.table = 0
            raise Error("lancedb.open: " + _last_error(self.lib))

        var name_c = _cstr(table)
        var tbl_fn = self.lib.get_function[Int]("ldb_table")
    ...
    def delete(self, predicate: String) raises:
        ...
        var f = self.lib.get_function[c_int]("ldb_delete")
    ...
    def optimize(self) raises:
        ...
        var f = self.lib.get_function[c_int]("ldb_optimize")
    ...
    def create_index(self) raises:
        ...
        var f = self.lib.get_function[c_int]("ldb_create_index")
        if Int(f(self.table)) != 0:
            raise Error("lancedb.create_index: " + _last_error(self.lib))

    def count(self) raises -> Int:
        """Number of rows in the table."""
        var func = self.lib.get_function[Int64]("ldb_count")
...
def _add(
    ...
    var func = lib.get_function[c_int]("ldb_add")
    var rc = Int(
        func(table, Int(ids.unsafe_ptr()), Int(vectors.unsafe_ptr()), n, dim)
    )
    if rc != 0:
...
def _search(
    ...
    var func = lib.get_function[c_int]("ldb_search")
    ...
    var got = Int(
        func(
            table,
            Int(query.unsafe_ptr()),
            dim,
            k,
            Int(out_ids.unsafe_ptr()),
            Int(out_dists.unsafe_ptr()),
        )
    )
    if got < 0:
